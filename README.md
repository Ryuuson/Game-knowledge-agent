...
默认检索会用本地 BGE 模型编码问题和知识块，再取语义最接近的片段作为回答依据。相似度不足时，系统不会把不相关的游戏资料套到问题上；边界结果会结合问题的游戏语境进一步判断。
...
联网搜索默认关闭。当前内置适配器使用 Metaso。配置 `METASO_API_KEY` 后，Agent 才会在本地资料不足或问题需要最新公开信息时联网搜索。使用联网搜索的回答会明确说明这一点。
